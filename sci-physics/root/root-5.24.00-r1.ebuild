...
IUSE="afs castor cern cint7 clarens doc emacs examples fftw geant4 kerberos ldap
	+math mysql	odbc openmp oracle postgres pythia6 pythia8 python +reflex rfio
...
src_prepare() {
	...
	epatch "${WORKDIR}"/${P}-g4root-flags.patch
        epatch "${FILESDIR}"/${PN}-5.22.00-Makefile-noh2root.diff
        epatch "${FILESDIR}"/${PN}-5.22.00-Module.mk-noh2root.diff
